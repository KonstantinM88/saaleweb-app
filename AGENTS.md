...
- `public/images/sections/` - static optimized WebP assets for non-record-specific homepage/landing sections.
...
- The homepage `GrowthWindow` section sits between `Industries` and `LocalSeo` to bridge industry specialization into regional visibility with a premium visual pause. Keep it CSS-first and image-light: optimized WebP, desktop CSS fixed-background parallax, mobile `next/image`, no canvas/video/heavy scroll JS. Keep large text outside the image so it does not hide the illustration.
