...
- `src/widgets/services-page/ServiceTrustMetrics.tsx` - interactive responsive performance/trust panel on the service index hero; desktop uses pointer-aware tilt/glow, mobile uses a compact 2x2 layout, and all motion respects reduced-motion preferences.
...
- The service index trust metrics use `ServiceTrustMetrics`; keep the semantic `dl` structure, mobile 2x2 layout, fine-pointer-only tilt, and reduced-motion behavior when changing this block.
...
- Service detail and index pricing links must be generated with `getPathname({locale, href: "/preise"})`; do not construct localized `/preise` URLs manually because EN/RU public aliases are `/pricing` and `/tseny`.
