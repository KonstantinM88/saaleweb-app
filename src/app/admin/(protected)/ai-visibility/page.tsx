...
import { adminCopy, type AdminLocale } from "@/features/admin/i18n";
...
function statusLabel(locale: AdminLocale, check?: { mentioned: boolean; cited: boolean } | null) {
  if (!check) return { text: adminCopy(locale, "Offen", "Не проверено"), className: "bg-gray-100 text-gray-600" };
  if (check.cited) return { text: adminCopy(locale, "Link vorhanden", "Есть ссылка"), className: "bg-emerald-50 text-emerald-700" };
  if (check.mentioned) return { text: adminCopy(locale, "Erwähnt", "Упомянут"), className: "bg-violet-50 text-violet-700" };
  return { text: adminCopy(locale, "Nicht gefunden", "Не найден"), className: "bg-red-50 text-red-700" };
}
...
export default async function AiVisibilityPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <>
      <PageHeader
        title={t("AI-Sichtbarkeit", "AI-видимость")}
        subtitle={t(
          "Wöchentlicher, reproduzierbarer Benchmark für 20 kommerzielle Suchfragen.",
          "Еженедельный воспроизводимый мониторинг 20 коммерческих запросов.",
        )}
      />
      ...
      {!databaseReady ? (
        <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          {t("Die Datenbanktabelle fehlt oder ist nicht erreichbar. Bitte zuerst die Migration anwenden.", "Таблица базы данных отсутствует или недоступна. Сначала примените миграцию.")}
        </div>
      ) : null}

      <div className="mb-6 grid gap-4 md:grid-cols-4">
        {[
          { label: t("Geprüft", "Проверено"), value: `${checked}/${total}`, tone: "text-dark" },
          { label: t("Erwähnungen", "Упоминания"), value: `${mentioned}/${total}`, tone: "text-violet-700" },
          { label: t("Zitate mit Link", "Ссылки на сайт"), value: `${cited}/${total}`, tone: "text-emerald-700" },
          { label: t("Abdeckung", "Покрытие"), value: `${Math.round((checked / total) * 100)} %`, tone: "text-brand-purple" },
        ].map((item) => (
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              {t(
                `Meldet ${new Set(AI_VISIBILITY_PROMPTS.map((item) => item.targetPath)).size} priorisierte Zielseiten an Bing und teilnehmende Suchsysteme.`,
            ...
            <p className="mt-2 text-xs text-muted">
              {t("Status", "Статус")}: <span className={isIndexNowConfigured() ? "font-semibold text-emerald-700" : "font-semibold text-amber-700"}>
                {isIndexNowConfigured() ? t("konfiguriert", "настроен") : t("INDEXNOW_KEY fehlt", "INDEXNOW_KEY отсутствует")}
              </span>
            </p>
          </div>
          <form action={submitPriorityUrlsToIndexNow}>
            ...
            <button type="submit" className={adminBtn} disabled={!isIndexNowConfigured()}>
              {t("Prioritätsseiten melden", "Отправить приоритетные страницы")}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        <Link href={`/admin/ai-visibility?week=${shiftIsoWeek(week, -1)}`} className={adminBtnGhost}>← {t("Vorwoche", "Прошлая неделя")}</Link>
        <span className="rounded-lg bg-brand-soft px-4 py-2 text-sm font-bold text-brand-purple">{week}</span>
        <Link href={`/admin/ai-visibility?week=${shiftIsoWeek(week, 1)}`} className={adminBtnGhost}>{t("Folgewoche", "Следующая неделя")} →</Link>
        {week !== getIsoWeekId() ? <Link href="/admin/ai-visibility" className={adminBtnGhost}>{t("Aktuelle Woche", "Текущая неделя")}</Link> : null}
      ...
      <div className={`${adminCard} mb-6 overflow-x-auto`}>
        <table className="w-full min-w-[980px] text-sm">
          <thead className="border-b border-line bg-surface text-left text-xs uppercase tracking-wide text-muted">
            <tr>
              <th className="px-4 py-3">{t("Suchfrage / Zielseite", "Запрос / целевая страница")}</th>
            ...
            {AI_VISIBILITY_PROMPTS.map((prompt, index) => (
              <tr key={prompt.key} className="border-b border-line align-top last:border-0">
                <td className="max-w-xl px-4 py-4">
                  <div className="text-xs font-bold text-brand-purple">{String(index + 1).padStart(2, "0")} · {locale === "ru" ? prompt.intent : prompt.intentDe}</div>
                ...
                {AI_VISIBILITY_PLATFORMS.map((platform) => {
                  ...
                  const badge = statusLabel(locale, check);
                  return (
                    <td key={platform.key} className="px-3 py-4">
                      ...
                      {check?.mentionOrder ? <div className="mt-1 text-xs text-muted">{t("Position", "Позиция")}: {check.mentionOrder}</div> : null}
                    </td>
                  );
                })}
              </tr>
        ...
        <section className={`${adminCard} scroll-mt-6 p-5`} id="result-form">
          <div className="flex flex-col gap-3 border-b border-line pb-4 md:flex-row md:items-start md:justify-between">
            ...
            <a href={selectedPlatform.href} target="_blank" rel="noreferrer" className={adminBtnGhost}>
              {t("Öffnen", "Открыть")} {selectedPlatform.label} ↗
            </a>
          </div>
          ...
          <form action={saveAiVisibilityCheck} className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label className="flex items-center gap-2 rounded-lg border border-line p-3 text-sm font-semibold text-ink">
              ...
              {t("SaaleWeb wird erwähnt", "SaaleWeb упоминается")}
            </label>
            <label className="flex items-center gap-2 rounded-lg border border-line p-3 text-sm font-semibold text-ink">
              <input type="checkbox" name="cited" defaultChecked={selectedCheck?.cited ?? false} className="size-4 accent-emerald-600" />
              {t("SaaleWeb wird als Quelle verlinkt", "Есть ссылка на SaaleWeb как на источник")}
            </label>

            <label className={adminLabel}>
              {t("Ungefähre Position (optional)", "Примерная позиция (необязательно)")}
          ...
          {selectedCheck ? (
            <form action={deleteAiVisibilityCheck} className="mt-3">
              ...
              <input type="hidden" name="promptKey" value={selectedPrompt.key} />
              <input type="hidden" name="platform" value={selectedPlatform.key} />
              <button type="submit" className="text-xs font-semibold text-red-600 hover:underline">{t("Ergebnis löschen", "Удалить результат")}</button>
            </form>
          ) : null}
        </section>
      ) : (
        <div className="rounded-xl border border-dashed border-line bg-surface px-5 py-8 text-center text-sm text-muted">
          {t("Wählen Sie in der Tabelle den Plattformstatus aus, um ein Prüfergebnis zu erfassen.", "Выберите статус платформы в таблице, чтобы записать результат проверки.")}
        </div>
      )}
    </>
  );
