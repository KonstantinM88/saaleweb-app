...
function statusLabel(check?: { mentioned: boolean; cited: boolean } | null) {
  if (!check) return { text: "Offen", className: "bg-gray-100 text-gray-600" };
  if (check.cited) return { text: "Zitiert", className: "bg-emerald-50 text-emerald-700" };
  if (check.mentioned) return { text: "Erwähnt", className: "bg-violet-50 text-violet-700" };
  return { text: "Nicht genannt", className: "bg-red-50 text-red-700" };
}
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Wöchentlicher, reproduzierbarer Benchmark für 20 kommerzielle Suchfragen."
      />
      ...
      {status ? (
        <div className={`mb-5 rounded-xl border px-4 py-3 text-sm ${
          ...
          {status === "saved" && "Prüfergebnis gespeichert."}
          {status === "deleted" && "Prüfergebnis entfernt."}
          {status === "invalid" && "Die Eingaben waren ungültig. Bitte Felder und URL prüfen."}
          {status === "indexnow-missing" && "IndexNow ist noch nicht konfiguriert. INDEXNOW_KEY fehlt oder ist ungültig."}
          {status.startsWith("indexnow-sent") && `${status.split("-").at(-1)} Prioritätsseiten wurden an IndexNow gemeldet.`}
          {status.startsWith("indexnow-failed") && "IndexNow hat die Anfrage nicht angenommen. Status und Hosting-Logs prüfen."}
        </div>
      ) : null}

      {!databaseReady ? (
        <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          Die Datenbanktabelle fehlt oder ist nicht erreichbar. Migration zuerst anwenden.
        </div>
      ) : null}

      <div className="mb-6 grid gap-4 md:grid-cols-4">
        {[
          { label: "Geprüft", value: `${checked}/${total}`, tone: "text-dark" },
          { label: "Erwähnungen", value: `${mentioned}/${total}`, tone: "text-violet-700" },
          { label: "Zitate mit Link", value: `${cited}/${total}`, tone: "text-emerald-700" },
          { label: "Abdeckung", value: `${Math.round((checked / total) * 100)} %`, tone: "text-brand-purple" },
        ].map((item) => (
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Meldet die {new Set(AI_VISIBILITY_PROMPTS.map((item) => item.targetPath)).size} priorisierten Zielseiten an Bing und teilnehmende Suchsysteme.
            </p>
            <p className="mt-2 text-xs text-muted">
              Status: <span className={isIndexNowConfigured() ? "font-semibold text-emerald-700" : "font-semibold text-amber-700"}>
                {isIndexNowConfigured() ? "konfiguriert" : "INDEXNOW_KEY fehlt"}
              </span>
            </p>
          </div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        <Link href={`/admin/ai-visibility?week=${shiftIsoWeek(week, -1)}`} className={adminBtnGhost}>← Vorwoche</Link>
        <span className="rounded-lg bg-brand-soft px-4 py-2 text-sm font-bold text-brand-purple">{week}</span>
        <Link href={`/admin/ai-visibility?week=${shiftIsoWeek(week, 1)}`} className={adminBtnGhost}>Folgewoche →</Link>
        {week !== getIsoWeekId() ? <Link href="/admin/ai-visibility" className={adminBtnGhost}>Aktuelle Woche</Link> : null}
      ...
      <div className={`${adminCard} mb-6 overflow-x-auto`}>
        <table className="w-full min-w-[980px] text-sm">
          <thead className="border-b border-line bg-surface text-left text-xs uppercase tracking-wide text-muted">
            <tr>
              <th className="px-4 py-3">Suchfrage / Zielseite</th>
                ...
                {AI_VISIBILITY_PLATFORMS.map((platform) => {
                  ...
                  return (
                    <td key={platform.key} className="px-3 py-4">
                      ...
                      {check?.mentionOrder ? <div className="mt-1 text-xs text-muted">Position: {check.mentionOrder}</div> : null}
                    </td>
                  );
                })}
              </tr>
      ...
      {selectedPrompt && selectedPlatform ? (
        <section className={`${adminCard} scroll-mt-6 p-5`} id="result-form">
          ...
          <form action={saveAiVisibilityCheck} className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label className={adminLabel}>
              Auffälliger Wettbewerber (optional)
            ...
            <div className="flex flex-wrap gap-2 md:col-span-2">
              <button type="submit" className={adminBtn}>Ergebnis speichern</button>
            </div>
          </form>
          ...
          {selectedCheck ? (
            <form action={deleteAiVisibilityCheck} className="mt-3">
              ...
              <input type="hidden" name="promptKey" value={selectedPrompt.key} />
              <input type="hidden" name="platform" value={selectedPlatform.key} />
              <button type="submit" className="text-xs font-semibold text-red-600 hover:underline">Eintrag löschen</button>
            </form>
          ) : null}
        </section>
      ) : (
        <div className="rounded-xl border border-dashed border-line bg-surface px-5 py-8 text-center text-sm text-muted">
          Einen Plattform-Status in der Tabelle auswählen, um das Prüfergebnis zu erfassen.
        </div>
      )}
    </>
  );
