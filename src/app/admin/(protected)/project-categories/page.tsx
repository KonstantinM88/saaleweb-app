...
import { deleteProjectCategory } from "@/features/admin/project-categories/actions";
export const dynamic = "force-dynamic";
type Row = { id: string; key: string; translations: { locale: string; name: string }[] };
async function getRows(): Promise<Row[]> { try { return (await prisma.projectCategory.findMany({ include: { translations: true } })) as Row[]; } catch { return []; } }
export default async function ProjectCategoriesAdminPage() {
  const rows = await getRows();
  return (<>
    <PageHeader title="Projekt-Kategorien" subtitle="Kategorien für Projekte/Cases (DE/EN/RU)." actionHref="/admin/project-categories/new" actionLabel="Neu" />
      ...
      <tbody>
        ...
        {rows.map((r) => { const de = r.translations.find((t) => t.locale === "de") ?? r.translations[0]; return (
          <tr key={r.id} className="border-b border-line last:border-0">
            ...
            <td className="px-4 py-3"><div className="flex justify-end gap-2"><Link href={`/admin/project-categories/${r.id}`} className={adminBtnGhost}>Bearbeiten</Link><form action={deleteProjectCategory.bind(null, r.id)}><ConfirmButton message="Kategorie löschen?" className="rounded-lg border border-red-200 bg-white px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50">Löschen</ConfirmButton></form></div></td>
          </tr>); })}
      </tbody></table></div>
  </>);
}
