...
import { IMPRESSUM_CONTENT } from "@/widgets/legal/legalContent";
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const content = IMPRESSUM_CONTENT[locale as AppLocale];
  const languages = Object.fromEntries(
    routing.locales.map((targetLocale) => [
      targetLocale,
      getPathname({ locale: targetLocale, href: "/impressum" }),
    ]),
  );
...
export default async function ImpressumPage({ params }: { params: Promise<Params> }) {
  ...
  const pagePath = getPathname({ locale: locale as AppLocale, href: "/impressum" });
