  ...
  try {
    const rows = (await prisma.serviceTranslation.findMany({
      ...
      select: {
        ...
        slug: true,
        excerpt: true,
        service: { select: { coverImage: true } },
      },
    })) as {
      name: string;
      slug: string;
      excerpt: string | null;
      service: { coverImage: string | null };
    }[];
    ...
    <>
      ...
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: tp("servicesLabel"), path: getPathname({ locale, href: "/leistungen" }) },
        ])}
      />
      <main>
            ...
            {items.length === 0 ? (
              ...
              <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((s) => (
                  <Link
                    key={s.slug}
                    href={{ pathname: "/leistungen/[slug]", params: { slug: s.slug } }}
                  ...
                  >
                    ...
                    <span className="mt-4 text-sm font-semibold text-brand-purple">{t("more")} →</span>
                  </Link>
                ))}
              </div>
            )}
