  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Zum Schutz unserer öffentlichen Anfrageformulare vor automatisiertem Spam und Missbrauch nutzen wir Cloudflare Turnstile (Cloudflare, Inc., 101 Townsend St, San Francisco, CA 94107, USA). Dabei werden technische Verbindungs-, Browser-, Geräte- und Interaktionsdaten einschließlich der IP-Adresse an Cloudflare übermittelt, um menschliche Eingaben von automatisierten Zugriffen zu unterscheiden. Das erzeugte Prüftoken wird einmalig serverseitig validiert; wir speichern die dafür übermittelte IP-Adresse nicht zusammen mit Ihrer Anfrage. Die Verarbeitung ist für die sichere Bereitstellung der Formulare erforderlich und beruht auf unserem berechtigten Interesse an Missbrauchsschutz gemäß Art. 6 Abs. 1 lit. f DSGVO. Für mögliche Übermittlungen in die USA gelten die von Cloudflare bereitgestellten Datenschutz- und Übermittlungsmechanismen. Weitere Informationen: https://www.cloudflare.com/privacypolicy/.",
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "To protect our public inquiry forms from automated spam and misuse, we use Cloudflare Turnstile (Cloudflare, Inc., 101 Townsend St, San Francisco, CA 94107, USA). Technical connection, browser, device and interaction data, including the IP address, is transmitted to Cloudflare to distinguish human submissions from automated access. The resulting verification token is validated once on the server; we do not store the IP address transmitted for this check together with your inquiry. Processing is necessary to provide the forms securely and is based on our legitimate interest in preventing misuse under Art. 6(1)(f) GDPR. Cloudflare's data protection and transfer mechanisms apply to possible transfers to the United States. More information: https://www.cloudflare.com/privacypolicy/.",
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Для защиты публичных форм от автоматического спама и злоупотреблений мы используем Cloudflare Turnstile (Cloudflare, Inc., 101 Townsend St, San Francisco, CA 94107, США). Cloudflare получает технические данные соединения, браузера, устройства и взаимодействия, включая IP-адрес, чтобы отличить действия человека от автоматизированных запросов. Полученный проверочный токен однократно валидируется на сервере; переданный для проверки IP-адрес не сохраняется нами вместе с заявкой. Обработка необходима для безопасной работы форм и основана на нашем законном интересе в защите от злоупотреблений согласно ст. 6 (1)(f) GDPR. При возможной передаче данных в США применяются механизмы защиты и передачи данных Cloudflare. Подробнее: https://www.cloudflare.com/privacypolicy/.",
