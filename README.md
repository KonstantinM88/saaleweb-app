...
```bash
# 1. Configure environment (REQUIRED before install because postinstall runs Prisma)
cp .env.example .env
#   then set DATABASE_URL to your PostgreSQL instance

# 2. Install dependencies (also generates the Prisma 7 client into src/generated/prisma)
npm install

# 3. Generate manually only if install scripts were skipped
# npm run db:generate
...
- **Prisma 7**: Rust-free `prisma-client` generator with a required `output`
  (`src/generated/prisma`, git-ignored — generated by `postinstall` / `db:generate`). Connection config lives in
...
- `npm install` runs `"postinstall": "prisma generate"` so deployments such as Vercel create
  `src/generated/prisma` before `next build`. Vercel must have `DATABASE_URL` configured.
...
src/
  ...
  generated/prisma/    # generated client — created by postinstall / `npm run db:generate`
```
